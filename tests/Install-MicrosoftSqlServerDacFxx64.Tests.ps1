...
InModuleScope "PoshSSDTBuildDeploy" {
    ...
    Describe "Install-MicrosoftSqlServerDacFxx64" {
        ...
        context "Old version of dotnet" {
            # Mock to ensure an old version of dotnet is returned
            mock Test-NetInstalled {
                $DotNetInfo = @{ DotNetVersion = 0 ; DWORD = 0; RequiredVersion = $false}
                return $DotNetInfo
            }
        
            It "Should throw if old version of dotnet installed" {
                {Install-MicrosoftSqlServerDacFxx64 -WorkingFolder $WWI -DacFxx64Version "130.3450.1"} | Should -Throw "Need to install .NET 4.6.1 at least!"
                Assert-MockCalled Test-NetInstalled -Exactly 1
            }
        }
